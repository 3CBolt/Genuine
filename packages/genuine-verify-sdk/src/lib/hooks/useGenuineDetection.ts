...
export function useGenuineDetection(options?: GenuineDetectionOptions & {
  ...
  // Add tilt buffer ref with explicit typing
  const tiltBuffer = useRef<TiltBuffer>({
    angles: [] as number[],
    directions: [] as ('left' | 'right' | 'none')[],
  ...
  const calculateHeadTiltMetrics = useCallback((leftEyeRaw: any, rightEyeRaw: any): HeadTiltMetrics => {
    ...
    // Update tilt buffer with explicit typing
    const currentBuffer = tiltBuffer.current;
    tiltBuffer.current = {
      angles: [...currentBuffer.angles.slice(-TILT_BUFFER_SIZE), tiltAngle],
      directions: [...currentBuffer.directions.slice(-TILT_BUFFER_SIZE), tiltDirection],
      timestamps: [...currentBuffer.timestamps.slice(-TILT_BUFFER_SIZE), Date.now()]
    }
